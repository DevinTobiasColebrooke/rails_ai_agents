...
### 2. End-to-End (E2E) Browser Testing & Manual Testing
When asked to test a specific user flow (e.g., "Test the checkout flow" or "Test user registration"), you must incorporate manual testing techniques. Manual testing MUST be used for ANY changes that are going to be seen or used by users or admin.
...
2. **Read Test Plan:** Use the `glob` and `read` tools to locate the relevant `TC-*.md` file in `docs/planning/test_cases/cases/`. Read the "Manual Testing Script (Playwright)" section to understand the exact manual steps required.
3. **Load Playwright Skill:** You MUST immediately call the `skill` tool with `name: "playwright-cli"`.
4. **Draft and Execute the Script:** Follow the instructions provided by the `playwright-cli` skill to write a robust test script (using the provided wrapper CLI or by generating an ad-hoc Node/Playwright script) to manually navigate the requested flow, click buttons, fill forms, and take screenshots for visual verification *exactly as outlined in the markdown manual testing script*. Ensure manual testing exploration is involved!
